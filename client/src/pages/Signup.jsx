...
import { UserPlus, Mail, Lock, User, Palette, Rocket, Star, AlertTriangle, Loader2, PartyPopper, KeyRound } from 'lucide-react';
...
export default function Signup() {
    ...
    return (
        <div className="min-h-screen bg-[#fdfdfd] dark:bg-[#0a0a0f] relative flex flex-col items-center justify-center p-6 font-sans selection:bg-yellow-200 dark:selection:bg-yellow-800 overflow-hidden transition-colors">
            ...
            <div className="absolute top-20 left-10 md:left-32 -rotate-12 opacity-20 dark:opacity-[0.15] hidden sm:block dark:text-white"><Palette size={52} strokeWidth={1.5} /></div>
            <div className="absolute bottom-20 right-10 md:right-32 rotate-12 opacity-20 dark:opacity-[0.15] hidden sm:block dark:text-white"><Rocket size={52} strokeWidth={1.5} /></div>
            <div className="absolute top-32 right-20 rotate-45 opacity-20 dark:opacity-[0.15] hidden md:block dark:text-white"><Star size={52} strokeWidth={1.5} /></div>
            ...
            <div className="w-full max-w-md bg-white dark:bg-gray-800 border-2 border-black dark:border-gray-600 rounded-2xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] dark:shadow-[8px_8px_0px_0px_rgba(55,65,81,1)] p-8 relative z-10 flex flex-col items-center rotate-1 hover:rotate-0 transition-transform duration-300">
                ...
                {/* Icon Header */}
                <div className="bg-green-300 dark:bg-green-700 border-2 border-black dark:border-gray-600 p-4 rounded-xl shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] -rotate-6 mb-6 hover:rotate-0 transition-transform">
                    <UserPlus size={36} strokeWidth={2.5} className="text-black dark:text-white" />
                </div>
                ...
                <form onSubmit={handleSubmit} className="w-full space-y-4">
                    ...
                    <div className="relative">
                        <User size={20} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400" strokeWidth={2.5} />
                        <input
                            type="text"
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="w-full bg-purple-300 dark:bg-purple-700 hover:bg-purple-400 dark:hover:bg-purple-800 text-black dark:text-white font-bold text-lg py-4 px-6 border-2 border-black dark:border-gray-600 rounded-xl flex items-center justify-center gap-3 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(55,65,81,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[4px] active:translate-y-[4px] transition-all disabled:opacity-60 disabled:pointer-events-none -rotate-1"
                    >
                        {isLoading ? <><Loader2 size={18} className="animate-spin" /> Creating account...</> : <><PartyPopper size={18} strokeWidth={2.5} /> Sign Up</>}
                    </button>
                </form>
                ...
                <div className="w-full relative flex items-center py-4 my-1">
                    <div className="flex-grow border-t-2 border-black dark:border-gray-600 border-dashed"></div>
                    <span className="flex-shrink-0 mx-4 text-black dark:text-white font-bold px-4 py-1 bg-blue-200 dark:bg-blue-800/60 border-2 border-black dark:border-gray-600 rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] rotate-2 text-sm">
                        Already a member?
                    </span>
                    <div className="flex-grow border-t-2 border-black dark:border-gray-600 border-dashed"></div>
                </div>
                ...
                >
                    <KeyRound size={18} strokeWidth={2.5} className="inline" /> Log In Instead
                </Link>
            </div>
