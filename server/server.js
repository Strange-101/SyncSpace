...
function getActiveHost(roomId) {
  ...
  if (roomMembers && roomMembers.size > 0) {
    ...
    console.log(`[Lobby] Auto-promoted ${userNames.get(newHost) || newHost} as host of ${roomId}`);
    return newHost;
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('request_join', ({ roomId, userName }) => {
    ...
    if (memberCount === 0 && !activeHost) {
      ...
      console.log(`[Lobby] ${userName} auto-admitted as first user (will become host on join_room)`);
  ...
  socket.on('admit_user', ({ roomId, targetSocketId }) => {
    ...
    const queue = waitingRoom.get(roomId) || [];
    waitingRoom.set(roomId, queue.filter(u => u.socketId !== targetSocketId));
    io.to(targetSocketId).emit('join_approved');
    console.log(`[Lobby] Host admitted ${userNames.get(targetSocketId) || targetSocketId} to room ${roomId}`);
  });
  ...
  socket.on('deny_user', ({ roomId, targetSocketId }) => {
    ...
    console.log(`[Lobby] Host denied ${userNames.get(targetSocketId) || targetSocketId} from room ${roomId}`);
  });
  ...
  socket.on('join_room', (data) => {
    ...
    socket.to(roomId).emit('user_joined', { id: socket.id, name: userNames.get(socket.id) || null });
    ...
    if (roomHosts.get(roomId) === socket.id) {
      const queue = waitingRoom.get(roomId) || [];
      queue.forEach(entry => {
        socket.emit('join_request', { socketId: entry.socketId, userName: entry.userName });
      });
    }
  });
    ...
    rooms.forEach(roomId => {
      if (roomId !== socket.id) {
        ...
        if (roomHosts.get(roomId) === socket.id) {
          const roomMembers = io.sockets.adapter.rooms.get(roomId);
          if (roomMembers) {
            const remaining = Array.from(roomMembers).filter(id => id !== socket.id);
            if (remaining.length > 0) {
              roomHosts.set(roomId, remaining[0]);
              console.log(`[Room] New host for ${roomId}: ${userNames.get(remaining[0]) || remaining[0]}`);
            } else {
              roomHosts.delete(roomId);
        ...
        const queue = waitingRoom.get(roomId) || [];
        waitingRoom.set(roomId, queue.filter(u => u.socketId !== socket.id));
      }
    });
  });
...
if (MONGO_URI && MONGO_URI !== 'your_mongodb_connection_string_here') {
  mongoose.connect(MONGO_URI, {
    ...
    .catch((err) => {
      ...
      server.listen(PORT, () => {
        console.log(`Server listening on port ${PORT} (no database - auth will NOT work)`);
      });
    });
} else {
  console.warn('⚠️  MONGO_URI not configured. Auth routes will NOT work. Set MONGO_URI in .env');
  server.listen(PORT, () => {
